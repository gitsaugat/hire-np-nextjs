...
const JobCard = ({ job, isActive, onClick }) => (
  <div 
    onClick={() => onClick(job)} 
    className={`group relative p-4 mb-3 transition-all duration-300 border rounded-[20px] cursor-pointer overflow-hidden ${
      isActive 
  ...
  >
    ...
    <div className="flex gap-4">
      <div className={`shrink-0 w-11 h-11 rounded-1.5xl flex items-center justify-center font-bold text-sm transition-all shadow-sm ${
        isActive ? "bg-[#0d4f3c] text-white rotate-3" : "bg-slate-50 text-[#0d4f3c] group-hover:bg-[#0f9e76] group-hover:text-white"
      }`}>
        {job.logoInitials}
      </div>
